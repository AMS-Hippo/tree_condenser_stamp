...
class NumbaTrainingForest:
    ...
    def from_compact_states(
        ...
        *,
        label_capacity: int,
        initial_label_sizes: list[int],
    ) -> "NumbaTrainingForest":
        """Flatten initial Python fit states into Numba-friendly arrays.

        ``initial_label_sizes`` contains the BPE fitting size published for
        each already-interned label id. Node ``size`` remains exact geometry.
        """
        ...
        if len(initial_label_sizes) > label_capacity:
            raise RuntimeError("Numba label capacity is too small")
        for label_id, fitting_size in enumerate(initial_label_sizes):
            if fitting_size <= 0:
                raise RuntimeError("BPE fitting sizes must be positive")
            label_size[label_id] = int(fitting_size)
        ...
        for current_tree, state in enumerate(states):
            ...
            for local_node in range(n):
                ...
                if label_size[label_id] <= 0:
                    raise RuntimeError(
                        "missing label fitting size for an interned label"
                    )
