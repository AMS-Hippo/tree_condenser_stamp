...
Fitting consumes topology and opaque node ``label`` values. Candidate
frequencies are raw counts of ``(parent_label, child_label)`` edges; attachment
maps and exact structural variants play no role in rule matching. Size-based
scores use one fitting size per label, while exact node ``size`` is retained for
transformation and decoding. During transformation the actual
occurrence attachment map is retained in an exact :class:`CompositeType`, so
stage-local decoding remains lossless.
"""
...
from ..schema import (
    ...
    set_graph_fitting_sizes,
)
from ..stage_decoder import StructuralStageDecoder
from ..structural import (
    CompositeType,
    infer_fitting_label_sizes,
    infer_input_alphabet,
...
class _CompactEdgeTree:
    ...
    def to_networkx(self, *, validate: bool = True) -> nx.DiGraph:
        ...
        set_graph_fitting_sizes(H, self.vocab.fitting_sizes)
        for old in live:
...
class EdgeBPEEncoder(TreeEncoder):
    ...
    def encode(self, G: nx.DiGraph, *, validate: bool = True) -> nx.DiGraph:
        ...
        codec = _TokenCodec()
        for label in sorted(self.base_labels, key=repr):
            codec.intern(label)
...
class EdgeBPECoarsener(TreeCoarsener):
    ...
    def _fit(self, graphs: Sequence[nx.DiGraph]) -> tuple[TreeEncoder, TreeDecoder]:
        ...
        # Exact geometry may vary within one fitting label (for example, a
        # variable-arity star family).  Keep fixed TokenSpecs where they exist,
        # but derive BPE scoring weights independently from exact geometry.
        fixed_input_alphabet = infer_input_alphabet(
            graphs,
            label_attr=self.label_attr,
            type_attr=self.type_attr,
            size_attr=self.size_attr,
        ...
        fitting_sizes = infer_fitting_label_sizes(
            graphs,
            label_attr=self.label_attr,
            type_attr=self.type_attr,
            size_attr=self.size_attr,
        )
        input_labels = frozenset(fitting_sizes)
        vocab = Vocabulary(
            symbols=fixed_input_alphabet,
        ...
        if use_numba:
            ...
            label_counts, label_sizes = _initial_label_statistics(
                states, codec, vocab
            )
        ...
        while self.num_merges is None or rank < self.num_merges:
            ...
            parent_spec = vocab.symbols.get(parent_label)
            child_spec = vocab.symbols.get(child_label)
            registered_fixed_spec = parent_spec is not None and child_spec is not None
            if registered_fixed_spec:
                vocab.add_symbol(
                    token,
                    TokenSpec(
